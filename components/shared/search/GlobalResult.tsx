...
const GlobalResult = () => {
  ...
  const [result, setResult] = useState([
    ...
    { type: "user", id: "3", title: "Hrishabh Joshi" },
  ]);
            ...
            {result.length > 0 ? (
              result.map((item: any, index: number) => (
                <Link
                  ...
                  className="flex w-full cursor-pointer items-start gap-3 px-5 py-2.5 hover:bg-light-700/50 hover:first-letter:dark:bg-dark-500/50"
                > 
                  <Image
                    alt="tag"
                    width={18}
